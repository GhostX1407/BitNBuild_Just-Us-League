...
import { LogoSplashAnimation } from '../../components/intro/LogoSplashAnimation';
...
export const LoginPage: React.FC = () => {
  ...
  // Splash animation runs on initial load / site visit
  const [showSplash, setShowSplash] = useState<boolean>(() => {
    return !sessionStorage.getItem('resqgrid_splash_completed');
  });
  ...
  const handleSplashComplete = () => {
    sessionStorage.setItem('resqgrid_splash_completed', 'true');
    setShowSplash(false);
  };
  ...
  return (
    <>
      {/* High-Tech Logo Splash Screen with Pure Vector Animation & Sound */}
      <AnimatePresence mode="wait">
        {showSplash && (
          <LogoSplashAnimation key="splash-intro" onComplete={handleSplashComplete} />
        )}
      </AnimatePresence>

      {/* Main Login Interface with Smooth Transition */}
      <motion.div
        initial={{ opacity: 0, y: 14 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
        className="min-h-screen w-full bg-slate-50 flex flex-col justify-between p-4 sm:p-8 select-none"
      >
        ...
        <header className="w-full max-w-5xl mx-auto flex items-center justify-between pb-6">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-white border border-slate-200/80 rounded-full text-xs font-medium text-slate-600 shadow-sm">
        ...
        <main className="w-full max-w-4xl mx-auto my-auto space-y-6">
          ...
          {mode === 'personas' ? (
            <div className="space-y-4">
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={cn('text-[10px] font-mono px-2.5 py-1 rounded-full font-bold shadow-xs', profile.badgeColor)}>
                          {profile.badgeLabel}
                        </span>
                      </div>
                      ...
                      {/* Scope Description */}
                      <p className="text-xs text-slate-600 font-sans leading-relaxed bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                      ...
                      <div className="space-y-1 pt-1">
                        <div className="flex flex-wrap gap-1">
                          {profile.features.slice(0, 3).map((feat, idx) => (
                            <span key={idx} className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 font-semibold border border-emerald-200">
                              ✓ {feat}
                            </span>
                          ))}
                        </div>
                        {profile.restrictedFeatures.length > 0 && (
                          <div className="flex flex-wrap gap-1 pt-0.5">
                            {profile.restrictedFeatures.slice(0, 2).map((rf, idx) => (
                              <span key={idx} className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-400 font-medium border border-slate-200 line-through">
                                ✕ {rf}
                              </span>
                            ))}
                          </div>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-700">
                      ...
                      <span className="inline-flex items-center gap-1 group-hover:translate-x-1 transition-transform text-blue-600">
                        <span>Log In As {profile.roleTitle.split(' ')[0]}</span>
              ...
              {/* Quick Access Matrix Pill */}
              <div className="p-3 bg-white border border-slate-200/80 rounded-2xl shadow-sm text-xs text-slate-600 flex flex-col sm:flex-row items-center justify-between gap-3 px-4">
                <div className="flex items-center gap-2 font-medium">
                  <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                  <span className="font-semibold text-slate-900">Role-Based Access Control (RBAC):</span>
                  <span>Each persona receives distinct permissions & views.</span>
                </div>
                <div className="flex items-center gap-2 text-[11px] font-mono font-semibold">
                  <span className="px-2 py-0.5 bg-slate-900 text-white rounded">Admin: Full</span>
                  <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded">Field: HUD</span>
                  <span className="px-2 py-0.5 bg-rose-100 text-rose-800 rounded">Hospital: Surge</span>
                  <span className="px-2 py-0.5 bg-amber-100 text-amber-800 rounded">Citizen: SOS</span>
                </div>
              </div>
            </div>
          ) : (
